...
func TestJsonWebSignature2020TestVectorJWT(t *testing.T) {
	...
	err = verifier.VerifyJWT(credential0)
	assert.NoError(t, err)
	...
	err = verifier.VerifyJWT(credential1)
	assert.NoError(t, err)
	...
	err = verifier.VerifyJWT(credential2)
	assert.NoError(t, err)
	...
	err = verifier.VerifyJWT(presentation0)
	assert.NoError(t, err)
	...
	err = verifier.VerifyJWT(presentation1)
	assert.NoError(t, err)

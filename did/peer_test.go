...
func TestPeerResolver(t *testing.T) {
	...
	_, err = r.Resolve(context.TODO(), m2, nil)
	assert.NoError(t, err)
	...
	_, err = r.Resolve(context.TODO(), m1, nil)
	assert.Error(t, err)
}

...
func TestJsonWebSignature2020TestVectorJWT(t *testing.T) {
	...
	err = verifier.VerifyVerifiableCredentialJWT(credential0)
	assert.NoError(t, err)
	...
	err = verifier.VerifyVerifiableCredentialJWT(credential1)
	assert.NoError(t, err)
	...
	err = verifier.VerifyVerifiableCredentialJWT(credential2)
	assert.NoError(t, err)
	...
	err = verifier.VerifyVerifiablePresentationJWT(presentation0)
	assert.NoError(t, err)
	...
	err = verifier.VerifyVerifiablePresentationJWT(presentation1)
	assert.NoError(t, err)
	...
	err = verifier.VerifyVerifiablePresentationJWT(presentation2)
	assert.NoError(t, err)
}
